...
def signup(request):
    if request.method == "GET":
        ...
        if request.POST['checker'] == "signup" and not checkifexists(request.POST['email']) and not checkifexistsmob(request.POST["phone"]):            
            ...
            return render(request, "home/static/templates/Signup/createacc.html", {"firstname": request.POST["first_name"], "lastname": request.POST["last_name"], "dob": request.POST["birthdate"], "gender": request.POST["gender"], "email": request.POST["email"], "adminmobile": request.POST["adminmobile"] , "privatedata1": request.POST["privatedata1"], "checker": "createacc"})
        elif request.POST['checker'] == "createacc" and not checkifexists(request.POST['email']):
            data = {"admin_name":str(request.POST["first_name"] + " " + request.POST["last_name"]),"admin_dob":str(request.POST["birthday"]),"admin_gender":str(request.POST["gender"]),"admin_email":str(request.POST["email"]),"admin_mobile":int(request.POST["adminmobile"]),"admin_pwd":str(request.POST["privatedata1"]),"clerk_name":str(request.POST["clerkname"]),"clerk_id":str(request.POST["clerkid"]),"clerk_pwd":str(rc4(request.POST["clerkpwd"],request.POST["clerkid"]))}
            admindata = Administrator(admin_name=data["admin_name"], admin_dob=data["admin_dob"],admin_gender=data["admin_gender"],admin_mobile=data["admin_mobile"],admin_email=data["admin_email"],admin_pwd=data["admin_pwd"],clerk_name=data["clerk_name"],clerk_id=data["clerk_id"],clerk_pwd=data["clerk_pwd"])
